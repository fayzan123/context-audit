...
check("windowEnd is the newest line", h.windowEnd === "2026-07-02T09:01:00.000Z", h.windowEnd);

// --- aggregation (must not regress) ----------------------------------------
console.log("AGGREGATION:");
{
  const imp = usage.get("impeccable");
  check("impeccable counts main + sidechain fires", imp?.invocations === 4, JSON.stringify(imp));
  ...
  check("first/last fired span the window", imp?.firstFired === "2026-07-01T10:00:00.000Z" && imp?.lastFired === "2026-07-01T10:20:00.000Z");
...
console.log("EVENTS:");
{
  check("one event per invocation with a parseable line", (h.events ?? []).length === 10, String(h.events?.length));
...
console.log("LEDGER ROUND-TRIP + DETERMINISM:");
{
  ...
  check("all events append to the ledger", r1.appended === 10 && r1.skipped === 0, JSON.stringify(r1));
  const r2 = openLedger(base).appendEvents(h.events ?? []);
  check("a second scan is a no-op (dedupe ids hold)", r2.appended === 0 && r2.skipped === 10, JSON.stringify(r2));
